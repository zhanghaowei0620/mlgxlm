...
class IndexController extends Controller
{
    ...
    public function index_coupon(Request $request){
        $couponInfo = DB::table('mt_goods')
            ...
            ->where('is_coupon',1)
            ->get(['mt_goods.goods_id','mt_shop.shop_id','mt_coupon.is_use','mt_shop.shop_name','mt_shop.shop_id','mt_goods.coupon_price','mt_goods.coupon_redouction','coupon_start_time','mt_goods.picture','mt_coupon.coupon_type','mt_goods.goods_name','mt_coupon.discount','mt_goods.picture']);
//        var_dump($couponInfo);
        if($couponInfo){
                $data = [
                    'code'=>0,
